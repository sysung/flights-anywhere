...
class TravelRecommendationService:
    ...
    def recommend(self, request: RecommendationRequest) -> RecommendationResponse:
        ...
        recommendations = self._recommendations(ranked, parsed.applied_filters, parsed.intent)
        assistant = self._assistant_message(recommendations, parsed.applied_filters)
        return RecommendationResponse(
    ...
    def _flexible_flight_requests(self, filters: TravelFilters) -> list[FlightSearchRequest]:
        ...
        current = start
        while current <= latest_departure and len(requests) < 8:
            requests.append(
                FlightSearchRequest(
                    origin=filters.origin or "SFO",
                    destination=filters.destination,
        ...
        for item in ranked[:8]:
            ...
            recommendations.append(
                DestinationRecommendation(
                    ...
                    tags=item.tags,
                    why=self._why(flight, item.tags, weather, places),
                    weather=weather,
                    places=places,
                    flight=flight,
                )
    ...
    def _assistant_message(self, recommendations: list[DestinationRecommendation], filters: TravelFilters) -> str:
